...
@main.route('/reports')
def reports():
    # Собираем статистику для отчетов
    stats = {
        'halls_count': ProductionHall.query.count(),
        'employees_count': Employee.query.count(),
        'items_in_progress': Item.query.filter_by(status=ItemStatusEnum.in_progress).count(),
        'items_completed': Item.query.filter_by(status=ItemStatusEnum.completed).count(),
        'workers_count': Worker.query.count(),
        'engineers_count': Engineer.query.count(),
        'teams_count': WorkTeam.query.count(),
        'laboratories_count': TestingLaboratory.query.count()
    }
    return render_template('reports/index.html', stats=stats)

@main.route('/reports/production_summary')
def production_summary():    # Статистика по цехам
    hall_stats = db.session.query(
        ProductionHall.name,
        db.func.count(Item.id).label('item_count')
    ).outerjoin(
        Item, ProductionHall.id == Item.hall_id
    ).group_by(ProductionHall.id, ProductionHall.name).all()
    
    # Статистика по статусам изделий
    status_stats = db.session.query(
        Item.status,
        db.func.count(Item.id).label('count')
    ).group_by(Item.status).all()
      # Статистика по категориям рабочих
    worker_category_stats = db.session.query(
        Worker.category,
        db.func.count(Worker.employee_id).label('count')
    ).group_by(Worker.category).all()
    
    # Статистика по инженерским категориям
    engineer_category_stats = db.session.query(
        CategoryEngineer.name,
        db.func.count(Engineer.employee_id).label('count')
    ).join(
        Engineer, CategoryEngineer.id == Engineer.category_id
    ).group_by(CategoryEngineer.name).all()
    
    return render_template('reports/production_summary.html',
                         hall_stats=hall_stats,
                         status_stats=status_stats,
                         worker_category_stats=worker_category_stats,
                         engineer_category_stats=engineer_category_stats)

@main.route('/reports/personnel')
def personnel_report():
    ...
    hall_distribution = db.session.query(
        ProductionHall.name,
        db.func.count(Worker.employee_id).label('workers_count'),
        db.func.count(Engineer.employee_id).label('engineers_count')
    ).outerjoin(
        Worker, ProductionHall.id == Worker.hall_id
    ).outerjoin(
        Engineer, ProductionHall.id == Engineer.hall_id
    ).group_by(ProductionHall.id, ProductionHall.name).all()
...
@main.route('/reports/items_status')
def items_status_report():    # Детальная статистика по изделиям
    items_by_hall = db.session.query(
        ProductionHall.name,
        Item.status,
        db.func.count(Item.id).label('count')
    ).join(
        Item, ProductionHall.id == Item.hall_id
    ).group_by(ProductionHall.name, Item.status).all()
    
    # Последние обновления изделий
    recent_items = db.session.query(Item, TypeItem, ProductionHall).join(
        TypeItem, Item.type_id == TypeItem.id
    ).join(
        ProductionHall, Item.hall_id == ProductionHall.id
    ).order_by(Item.id.desc()).limit(10).all()
    
    return render_template('reports/items_status.html',
                         items_by_hall=items_by_hall,
