...
def shop_managers():
    ...
    halls = db.session.query(ProductionHall, Engineer, Employee).outerjoin(
        Engineer, ProductionHall.shop_manager_id == Engineer.employee_id
    ).outerjoin(
        Employee, Engineer.employee_id == Employee.id
    ).all()
    
    available_engineers = db.session.query(Engineer, Employee).join(
        Employee, Engineer.employee_id == Employee.id
...
def remove_shop_manager(hall_id):
    ...
    hall = ProductionHall.query.get_or_404(hall_id)
    hall.shop_manager_id = None
    db.session.commit()
    flash('Начальник цеха снят с должности!', 'success')
    return redirect(url_for('main.shop_managers'))

# Назначение начальников участков
@main.route('/assignments/area_managers')
def area_managers():
    """Список назначений начальников участков"""
    areas = db.session.query(ProductionArea, Engineer, Employee, ProductionHall).outerjoin(
        Engineer, ProductionArea.area_manager_id == Engineer.employee_id
    ).outerjoin(
        Employee, Engineer.employee_id == Employee.id
    ).join(
        ProductionHall, ProductionArea.hall_id == ProductionHall.id
...
def assign_area_manager():
    ...
    if area_id and engineer_id:
        area = ProductionArea.query.get_or_404(area_id)
        engineer = Engineer.query.get_or_404(engineer_id)
        
        # Проверяем, что инженер работает на этом участке
        if engineer.area_id == area_id:
            area.area_manager_id = engineer_id
            db.session.commit()
...
def team_leaders():
    ...
    teams = db.session.query(WorkTeam, Worker, Employee, ProductionArea, ProductionHall).outerjoin(
        Worker, WorkTeam.team_leader_id == Worker.employee_id
    ).outerjoin(
        Employee, Worker.employee_id == Employee.id
    ).join(
        ProductionArea, WorkTeam.area_id == ProductionArea.id
...
@main.route('/assignments/remove_team_leader/<int:team_id>')
def remove_team_leader(team_id):
    """Снять бригадира"""
    team = WorkTeam.query.get_or_404(team_id)
